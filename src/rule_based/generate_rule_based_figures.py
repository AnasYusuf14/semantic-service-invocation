...
RESULTS_FILE = BASE_DIR / "results" / "rule_based" / "rule_based_results.csv"
FIGURES_DIR = BASE_DIR / "figures" / "rule_based"
...
def generate_metric_accuracy_chart(rows):
    ...
    plt.ylim(0, 100)
    plt.ylabel("Accuracy (%)")
    plt.title("Rule-Based Accuracy by Metric")

    for i, value in enumerate(values):
        plt.text(i, value + 1, f"{value:.1f}%", ha="center")

    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "rule_based_metrics.png", dpi=300)
    plt.close()
...
def generate_domain_accuracy_chart(rows):
    ...
    plt.title("Rule-Based Intent Accuracy by Domain")

    for i, value in enumerate(values):
        plt.text(i, value + 1, f"{value:.1f}%", ha="center")

    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "rule_based_domain_accuracy.png", dpi=300)
    plt.close()
...
def generate_success_failure_pie(rows):
    ...
    plt.title("Rule-Based End-to-End Success vs Failure")
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "rule_based_success_failure.png", dpi=300)
    plt.close()
...
def main():
    ...
    print("Rule-Based figures generated successfully.")
